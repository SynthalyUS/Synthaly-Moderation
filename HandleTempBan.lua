...
script.Parent.Events.SetTempBan.Event:Connect(function(UserID, Duration, Reason, Moderator)
	...
	if not Username_Success then
		return warn("[!] Synthaly: Username couldn't be found while trying to set temporary ban: " ..Username_Error.. "| Error code: 205")
	end
	...
	local Success, Error = pcall(function()
		...
		messagingService:PublishAsync("Synthaly_TempBans", UserID)
	end)
	...
	if Success then
		...
		Log:Fire("User temporary banned", "A temporary ban was issued using Synthaly services. Duration of ban: `~"..TimeLeft.." minutes`. Moderated by **"..Moderator.."**."  , 5814783, "https://www.roblox.com/Thumbs/Avatar.ashx?x=100&y=100&username=" ..username, "User banned", username, "Reason", Reason, "New temporary ban log!")		
	end
	
	
	
	if not Success then
		warn("[!] Synthaly: Error while trying to create temporary ban: " ..Error.. "| Error code: 191")
	end
	...
	if game:GetService('Players'):FindFirstChild(username) then
		...
		if Theme == 'light' then
			...
			local GUI = script.Synthaly_TempBanned_Light:Clone()
			...
			if UseCustomGUI == true then
				...
				AClone.Background.MainFrame.LogOutButton.ClickButton.MouseButton1Click:Connect(function()
					toKick:Kick("| [TEMPORARY BANNED] | Time left: ~ " ..TimeLeft.. " minutes | Reason: " ..Reason.. " | -Synthaly")
				end)
		...
		elseif Theme == 'dark' then
			local GUI = script.Synthaly_TempBanned_Dark:Clone()
			...
			if EndTime > os.time() then
				...
				if Theme == 'light'  then
					...
					if UseCustomGUI == true then
						...
						AClone.Background.MainFrame.LogOutButton.ClickButton.MouseButton1Click:Connect(function()
							player:Kick("| [TEMPORARY BANNED] | Time left: ~ " ..TimeLeft.. " minutes | Reason: " ..Reason.. " | -Synthaly")
						end)
...
local messagingService = game:GetService("MessagingService")
messagingService:SubscribeAsync("Synthaly_TempBans", function(message)
	local targetName = game:GetService('Players'):GetNameFromUserIdAsync(message.Data) 
	print("[>] Synthaly: Received temporary ban request for " ..targetName)
