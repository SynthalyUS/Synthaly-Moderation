...
script.Parent.Events.SetSlock.Event:Connect(function(value, groupID, min_rank, moderator, reason)
	...
	if value == true then
		...
		Moderator = moderator
		Reason = reason
		print("[*] Synthaly: Server locked for players with the rank below " ..Min_Rank.." with reason: " ..Reason)
	...
	elseif value == false then
		...
		Moderator = nil
		Reason = nil
		print("[*] Synthaly: Server unlocked")
	...
	if IS_LOCKED == true then
		...
		elseif player:GetRankInGroup(Group_ID) < Min_Rank then
			...
			if Theme == 'light' then
				...
				local GUI = script.Synthaly_Slocked_Light:Clone()
					...
					player:Kick("| [SERVER LOCKED] | Reason: " ..Reason.. " | -Synthaly")
					end)
			...
			elseif Theme == 'dark' then
				local GUI = script.Synthaly_Slocked_Dark:Clone()
