...
script.Parent.Events.SetWarning.Event:Connect(function(UserID, Reason, Moderator)
	...
	if Success then
		...
		Log:Fire("New warning issued", "A new warning was issued using Synthaly services. Warning issued by **" ..Moderator.."**.", 5814783, "https://www.roblox.com/Thumbs/Avatar.ashx?x=100&y=100&username=" ..username, "Issued to", username, "Reason", Reason, "New warning log!")
	else
		warn("[!] Synthaly: Error while saving warning data: " ..Error.. "| Error code: 191")
	end
	...
	if Success then
		...
		if game:GetService('Players'):FindFirstChild(username) then
			...
			if Theme == 'light' then
				...
				local GUI = script.Synthaly_Warned_Light:Clone()
				...
				if UseCustomGUI == true then
					...
					GUI.Background.MainFrame.LogOutButton.ClickButton.MouseButton1Click:Connect(function()
						...
						API:removeWarning(player.Name, "Automatic warning removal: user accepted", "Synthaly Core")
						GUI:Destroy()
			...
			elseif Theme == 'dark' then
				local GUI = script.Synthaly_Warned_Dark:Clone()
...
game:GetService('Players').PlayerAdded:Connect(function(player)
	...
	if not Success then
		warn("[!] Synthaly: Error while getting database info: " ..Error.. "| Error code: 191")
	end
