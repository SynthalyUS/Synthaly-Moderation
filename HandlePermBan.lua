...
script.Parent.Events.SetPermBan.Event:Connect(function(UserID, Reason, Moderator)
	...
	local Success, Error = pcall(function()
		...
		messagingService:PublishAsync("Synthaly_PermBans", UserID)
	end)
	...
	if Success then
		...
		Log:Fire("User permanently banned", "A permanent ban was issued using Synthaly services. Moderated by **"..Moderator.."**."  , 5814783, "https://www.roblox.com/Thumbs/Avatar.ashx?x=100&y=100&username=" ..username, "User banned", username, "Reason", Reason, "New permanent ban log!")		
	end
	
	
	
	if not Success then
		warn("[!] Synthaly: Error while trying to create permanent ban: " ..Error.. "| Error code: 191")
	end
	...
	if game:GetService('Players'):FindFirstChild(username) then
		...
		if Theme == 'light' then
			...
			local GUI = script.Synthaly_PermBanned_Light:Clone()
			...
			if UseCustomGUI == true then
				...
				AClone.Background.MainFrame.LogOutButton.ClickButton.MouseButton1Click:Connect(function()
					toKick:Kick("| [PERMANENTLY BANNED] | Reason: " ..Reason.. " | -Synthaly")
				end)
		...
		elseif Theme == 'dark' then
			local GUI = script.Synthaly_PermBanned_Dark:Clone()
		...
		if Success and Data then
			...
			if Theme == 'light' then
				...
				if UseCustomGUI == true then
					...
					AClone.Background.MainFrame.LogOutButton.ClickButton.MouseButton1Click:Connect(function()
						player:Kick("| [PERMANENTLY BANNED] | Reason: " ..Reason.. " | -Synthaly")
					end)
...
local messagingService = game:GetService("MessagingService")
messagingService:SubscribeAsync("Synthaly_PermBans", function(message)
	local targetName = game:GetService('Players'):GetNameFromUserIdAsync(message.Data) 
	print("[>] Synthaly: Received ban request for " ..targetName)
